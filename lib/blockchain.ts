...
export async function initBlockchain() {
  try {
    ...
    if (!fs.existsSync(contractDataPath)) {
      console.warn(
        "contract_data.json not found. Blockchain features disabled."
      );
      return null;
    }
    ...
    provider = new ethers.JsonRpcProvider(getBlockchainRpcUrl());
    ...
    if (!privateKey) {
      console.error("Missing SEPOLIA_PRIVATE_KEY in environment variables");
      return null;
    }
...
export async function registerItem(
  ...
  try {
    ...
    console.error(
      "Blockchain write failed (registerItem):",
...
export async function verifyAndPay(
  ...
  try {
    ...
    if (!contract) await Promise.race([initBlockchain(), timeout]);
    if (!contract) return null;
    const tx = (await Promise.race([
      contract.verifyAndPay(itemId, finderAddress, secret, {
        gasLimit: 300000,
      }),
      timeout,
    ])) as any;
    ...
    console.error(
      "Blockchain write failed (verifyAndPay), will use offchain fallback:",
      (error as Error).message
    );
    return null;
  }
...
export async function getBlockchainStatus() {
  try {
    ...
    const readableNetwork = resolveReadableNetworkName(
      network.name,
      network.chainId
    );
...
export async function anchorExternalPayment(payload: {
  ...
  try {
    ...
    console.error(
      "Blockchain anchor failed (anchorExternalPayment):",
      (error as Error).message
    );
    return null;
  }
...
export async function anchorClaimDecision(payload: {
  ...
  try {
    ...
    console.error(
      "Blockchain anchor failed (anchorClaimDecision):",
      (error as Error).message
    );
    return null;
  }
...
export async function anchorClaimSettlement(payload: {
  ...
  try {
    ...
    console.error(
      "Blockchain anchor failed (anchorClaimSettlement):",
      (error as Error).message
    );
    return null;
  }
