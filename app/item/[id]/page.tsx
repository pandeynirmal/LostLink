  ...
  return (
    <div className='min-h-screen bg-background'>
      ...
      <main className='max-w-4xl mx-auto px-4 py-10 space-y-6'>
        <Card>
          ...
          <CardContent className='space-y-4'>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              {item.claimCount && item.claimCount > 0 && (
                <Badge variant="secondary" className="bg-blue-500/10 text-blue-600 border-blue-500/20">
                  {item.claimCount} Pending Claim{item.claimCount > 1 ? 's' : ''}
                </Badge>
              )}
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{isOwner ? 'Owner Information' : 'Claim Item'}</CardTitle>
          </CardHeader>
          <CardContent className='space-y-3'>
            {isOwner ? (
              ...
              <p className='text-sm text-muted-foreground'>
                To claim this item, please verify your identity and contact the owner.
              </p>
            )}

            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              <div className='space-y-1'>
                <p className='text-xs text-muted-foreground uppercase font-semibold'>Uploader Name</p>
                <p className='font-medium'>{item.user.fullName}</p>
              </div>
              <div className='space-y-1'>
                <p className='text-xs text-muted-foreground uppercase font-semibold'>Uploader Email</p>
                <p className='font-medium'>{item.user.email}</p>
              </div>
              {item.user.organization && (
                <div className='space-y-1'>
                  <p className='text-xs text-muted-foreground uppercase font-semibold'>Organization</p>
                  <p className='font-medium'>{item.user.organization}</p>
                </div>
              )}
              {item.contactPhone && (
                <div className='space-y-1'>
                  <p className='text-xs text-muted-foreground uppercase font-semibold'>Mobile</p>
            ...
            {!isOwner ? (
              <div className='pt-6 border-t border-border mt-6 space-y-4'>
                {item.isClaimed ? (
                  <div className='bg-green-500/10 p-4 rounded-lg border border-green-500/20'>
                    <p className='text-sm text-green-600 dark:text-green-400 font-medium'>
                      This item has already been successfully claimed and resolved.
                    </p>
                  </div>
                ) : (
                  <>
                    <div className='space-y-2'>
                      <label className='block text-sm font-medium'>Proposed Claim Amount (ETH)</label>
                      <p className='text-xs text-muted-foreground'>Enter the reward amount you expect or want to propose.</p>
                      <input
                        type='number'
                        min='0'
                        step='0.001'
                    ...
                    {item.claimCount && item.claimCount > 0 && (
                      <p className='text-xs text-amber-600 mt-2 italic text-center'>
                        Note: {item.claimCount} person(s) have already submitted claims for this item. 
                        You can still submit yours if you believe you are the rightful owner/finder.
                      </p>
                    )}
                  </>
                )}
                ...
                {requestStatus && (
                  <p className={`mt-2 text-sm font-medium ${requestStatus.includes('success') ? 'text-green-600' : 'text-red-600'}`}>
                    {requestStatus}
                  </p>
                )}
              </div>
