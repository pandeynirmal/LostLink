...
export async function GET(request: NextRequest) {
  try {
    ...
    const escrow = await EscrowCase.findOne({ itemId })
      .sort({ createdAt: -1 })
    ...
    // Allow any logged-in user to view escrow (read-only); actions are restricted in action route

    // Calculate auto-release status
    const now = Date.now();
    const autoReleaseAvailable = escrow.ownerItemReceived && 
                                  escrow.autoReleaseAt && 
                                  !escrow.autoReleaseTriggered &&
                                  now >= new Date(escrow.autoReleaseAt).getTime();
    
    const timeUntilAutoRelease = escrow.autoReleaseAt && !escrow.autoReleaseTriggered
      ? Math.max(0, new Date(escrow.autoReleaseAt).getTime() - now)
      : null;

    // Calculate release votes
    const releaseVotes = (escrow.ownerReleaseApproved ? 1 : 0) + 
    ...
    return NextResponse.json(
      { 
        ...
        meta: {
          ...
          isAdmin,
          releaseVotes,
          releaseReady: (escrow.ownerReleaseApproved && escrow.finderReleaseApproved) || (escrow.ownerItemReceived && releaseVotes >= 2),
          autoReleaseAvailable,
          timeUntilAutoRelease,
          canRaiseDispute: ["awaiting_delivery", "item_delivered", "awaiting_confirmation"].includes(escrow.state) && (isOwner || isFinder || isAdmin),
        }
      },
      { headers: { "Cache-Control": "no-store, no-cache, must-revalidate" } }
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { itemId, amountEth, holdTxHash = "", holdSource = "project_wallet" } = await request.json();
    if (!itemId) {
      return NextResponse.json({ error: "itemId is required" }, { status: 400 });
    }
    ...
    if (!isOwner && !isAdmin) {
      return NextResponse.json({ error: "Only owner/admin can setup escrow" }, { status: 403 });
    }

    const baseAmount = Number(item.rewardAmount || 0);
    const requestedAmount =
      Number.isFinite(Number(amountEth)) && Number(amountEth) > 0 ? Number(amountEth) : baseAmount;
    if (!Number.isFinite(requestedAmount) || requestedAmount <= 0) {
      return NextResponse.json({ error: "Escrow amount must be greater than 0" }, { status: 400 });
    }

    const existing = await EscrowCase.findOne({ itemId: item._id }).sort({ createdAt: -1 });
    ...
    if (existing && !["released", "refunded"].includes(existing.state)) {
      ...
      await existing.save();
      return NextResponse.json({ success: true, escrow: existing, message: "Escrow updated." });
    }
